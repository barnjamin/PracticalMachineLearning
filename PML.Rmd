...
```{r}
ds <- read.csv("~/Downloads/pml-training.csv")
```
...
Since there are over 19 thousand rows of data total, training would take a long time if we were to use the whole data set, so here we try with a sample of 500 at a time.  We'll demonstrate a few different algorithms on the same data to evaluate which to run the full training with. 
...
randomForrestTest <- train(classe~.,data=smpl1,method="rf")
(sum(smpl2$classe == predict(randomForrestTest, smpl2))/500)*100
confusionMatrix(randomForrestTest)
```

We can see we got about 85% correct on this small test for Random Forrest which is on par with what the researchers got as well.  With our algorithm chosen we can train it on the full testing data set then run it against the test data set to find out how well it does classification.

```{r, eval=FALSE}
rfTesting <- train(classe~.,data=subset,method="rf")

confusionMatrix(rfTesting)

testds <- read.csv("~/Downloads/pml-testing.csv")
prediction <- predict(rf, testds)
(sum(testds$classe==prediction)/length(testds))*100
```
